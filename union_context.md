...
- Competiciones por temporada en `SOFASCORE_COMPETITIONS_BY_SEASON` (`calendar_data.py`). En 2026/27:
  ...
  - `Primavera 1` (2292 / 97201), grupo `Grupo único`. En los informes aparece como `Italia (U20)`; la Primavera 2 no se carga (p. ej. Perugia U20 no tendrá partido).
  - Portugal: `Liga Portugal 2` (239 / 97470, `Grupo único`), `Liga 3` (17101 / 98660, `Group A/B` en la 1ª fase) y `Next Gen` = Liga Revelação U23 (11341 / 98717, `1ª fase`). Las fases posteriores de Liga 3 y Next Gen aparecerán cuando Sofascore las publique.
...
- `CalendarView`:
  - Selector `País` (banderas SVG: España, Italia, Francia, Portugal) → `Liga` del país → filtro multiselección de `Grupos`, antes de `Planificación de partidos`; solo aparecen países/ligas con partidos en la temporada y se muestra una liga cada vez. El país se deriva de la liga (`COMPETITION_COUNTRY`); estado persistido en `sessionStorage` (`calendar.competition`, `calendar.groups`).
  - Logos de liga (`assets/Logos/*.png`, mapa `COMPETITION_LOGOS`) en los botones de liga y en la cabecera negra de la sección, siempre sobre placa blanca para que los logos oscuros se lean. Una liga sin logo muestra solo el nombre. `RankingsView` también importa `1RFEF_Logo.png` / `2RFEF_Logo.png`: no renombrar esos archivos.
  ...
  - En las ligas extranjeras (`LOOSE_NAME_COMPETITIONS`) el emparejamiento ignora siglas de club (AC, US, SSC, CD, SAD, GD…), sufijos de edad (U19–U23) y años (`AC Trento` = `Trento`, `SSC Bari` = `Bari`); en 1RFEF/2RFEF se mantiene el mapa `TEAM_ALIASES` (National 1 solo necesita `ESTAC Troyes B` → `Troyes 2`).
